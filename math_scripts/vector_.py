...
class Vector(object):
    ...
    def plus(self, vectorB):
        return Vector(
            [x + y for x, y in zip(self.coordinates, vectorB.coordinates)])
    ...
    def dot(self, vectorB):
        return sum(
            [a * b for a, b in zip(self.coordinates, vectorB.coordinates)])
    ...
    def parallel(self, vectorB):
        return (self.is_zero() or vectorB.is_zero()
                or Util.is_nearly_zero(self.angle(vectorB)[0])
                or round(self.angle(vectorB)[0], 3) == round(math.pi, 3))
    ...
    def cross_product(self, vectorB):
        ...
        return Vector([
            y_1 * z_2 - y_2 * z_1, -(x_1 * z_2 - x_2 * z_1),
            x_1 * y_2 - x_2 * y_1
        ])
