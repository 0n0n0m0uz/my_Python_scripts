...
class Vector(object):
    ...
    def __str__(self):
        ...
        rounded_coords = [
            round(x, num_decimal_places) for x in self.coordinates
        ]
        return 'Vector: {}'.format(rounded_coords)

    def __eq__(self, v):
        coordinate_pairs = zip(self.coordinates, v.coordinates)
        coordinate_pair_equality = [
            MyDecimal(x - y).is_near_zero() for x, y in coordinate_pairs
        ]
    ...
    def plus(self, v):
        if self.dimension != v.dimension:
            raise ValueError(
                'The vectors to add must be in the same dimension')
    ...
    def minus(self, v):
        if self.dimension != v.dimension:
            raise ValueError(
                'The vectors to subtract must be in the same dimension')
    ...
    def angle_with(self, v, in_degrees=False):
        try:
            ...
            if str(e) == self.CANNOT_NORMALIZE_ZERO_VECTOR_MSG:
                raise Exception(
                    'Cannot calculate an angle with the zero vector')
            else:
                raise e

    def is_parallel_to(self, v):
        return (self.is_zero() or v.is_zero() or self.angle_with(v) == pi
    ...
    def component_orthogonal_to(self, b):
        try:
            ...
            if str(e) == self.CANNOT_FIND_UNIQUE_PARALLEL_COMPONENT_MSG:
                raise Exception(
                    self.CANNOT_FIND_UNIQUE_ORTHOGONAL_COMPONENT_MSG)
            else:
                raise e
