...
class Graphing(GraphScene):
    ...
    def construct(self):
        ...
        for i in range(len(ys)):
            points.append(
                Dot(self.coords_to_point((xs[i]), (ys[i])),
        ...
        second_line = TextMobject(
            "algoritmo es actualizar en $y = \\theta_{0} + \\theta_{1}*x$"
        ).scale(0.7)
        third_line = TextMobject(
            "los params $\\theta_{0}$ y $\\theta_{1}$ con las formulas:"
        ).scale(0.7)
        first_eq_theta0 = TextMobject(
            "$\\theta_0 := \\theta_0 - \\frac{\\partial J}{\\partial \\theta_0}$"
            + "$= {}$".format(np.round(self.b, 3))).scale(0.7)
        first_eq_theta1 = TextMobject(
            "$\\theta_1 := \\theta_1 - \\frac{\\partial J}{\\partial \\theta_1}$"
            + "$= {}$".format(np.round(self.a, 3))).scale(0.7)

        fourth_line = TextMobject("Para minimizar el costo $J$:").scale(0.7)
        second_eq = TextMobject("$J(\\theta_0, \\theta_1) = {}$".format(
            np.round(self.costo, 3))).scale(0.7)
        ...
        self.play(Write(first_line), Write(second_line), Write(third_line),
                  Write(first_eq_theta0), Write(first_eq_theta1),
                  Write(second_eq), Write(fourth_line))
        ...
        for i in range(0, num_iter):
            ...
            temp0 = theta[0] - (alpha / m) * sum(
                (np.dot(X, theta) - y) * X.iloc[:, 0])
            temp1 = theta[1] - (alpha / m) * sum(
                (np.dot(X, theta) - y) * X.iloc[:, 1])
            ...
            if (i % 1 == 0):
                ...
                func_graph2 = self.get_graph(self.func_to_graph,
                                             self.function_color)
                second_eq2 = TextMobject(
                    "$J(\\theta_0, \\theta_1) = {}$".format(
                        np.round(self.costo, 3))).scale(0.7)
                second_eq2.next_to(fourth_line, 3 * DOWN)
                first_eq_theta01 = TextMobject(
                    "$\\theta_0 := \\theta_0 - \\frac{\\partial J}{\\partial \\theta_0}$"
                    + "$= {} - $".format(np.round(theta[0], 3)) +
                    "$({})$".format(
                        np.round((alpha / m) * sum(
                            (np.dot(X, theta) - y) * X.iloc[:, 0]), 3)) +
                    "$={}$".format(np.round(temp0, 3))).scale(0.6)
                first_eq_theta01.next_to(third_line, DOWN * 3)
                first_eq_theta11 = TextMobject(
                    "$\\theta_1 := \\theta_1 - \\frac{\\partial J}{\\partial \\theta_1}$"
                    + "$= {} - $".format(np.round(theta[1], 3)) +
                    "$({})$".format(
                        np.round((alpha / m) * sum(
                            (np.dot(X, theta) - y) * X.iloc[:, 1]), 3)) +
                    "$={}$".format(np.round(temp1, 3))).scale(0.6)
                first_eq_theta11.next_to(first_eq_theta0, DOWN)
                self.play(Transform(func_graph1, func_graph2),
                          Transform(graph_lab, graph_lab2),
                          Transform(itertext1, itertext),
                          Transform(second_eq, second_eq2),
                          Transform(first_eq_theta0, first_eq_theta01),
                ...
                J = Dot(self.coords_to_point(i, self.costo),
