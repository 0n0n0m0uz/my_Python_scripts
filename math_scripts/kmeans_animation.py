...
if iscomparebruteforce == 1:
    ...
    np.random.seed(1)
    x = np.concatenate(
        (np.random.randn(dim, m) + np.tile(np.array([4, 1]), (m, 1)).T,
         np.random.randn(dim, m) + np.tile(np.array([4, 4]), (m, 1)).T),
        axis=1)
    x = np.concatenate(
        (x, np.random.randn(dim, m) + np.tile(np.array([1, 2]), (m, 1)).T),
        axis=1)
    ...
    x = np.concatenate(
        (x, np.tile(np.array([-7, -7]),
                    (10, 1)).T + 0.1 * np.random.randn(dim, 10)),
        axis=1)
    #number of clusters
    cno = 6
m = x.shape[1]
...
if iscomparebruteforce == 1:
    ...
    for i in range(1, m):
        ...
        for j in partition1:
            ...
            center1 = np.sum(
                x[:, group1_idx], axis=1) / float(group1_idx.shape[0])
            obj = obj + np.sum(
                np.sum(
                    np.power(
                        x[:, group1_idx] - np.tile(center1,
                                                   (group1_idx.shape[0], 1)).T,
                        2)))
            # equivalent:
            #center1.shape=(2,1)
            ...
            center2 = np.sum(
                x[:, group2_idx], axis=1) / float(group2_idx.shape[0])
            center2.shape = (2, 1)
            center2 = np.transpose(center2)
            for l in range(0, group2_idx.shape[0]):
                obj = obj + np.sum(
                    np.power(x[:, group2_idx[l]] - center2, 2), axis=1)
